...
function networkUp() {
  ...
  if [ ! -d "crypto-config" ]; then
    generateCerts
    #replacePrivateKey ##Commented since TLS is disabled on pharma network
    generateChannelArtifacts
  fi
  # Start the docker containers using compose file
  IMAGE_TAG=$IMAGETAG COMPOSE_HTTP_TIMEOUT=300 docker-compose -f "$COMPOSE_FILE" up -d 2>&1 #Added COMPOSE_HTTP_TIMEOUT to increase default timeout to 5 minutes
  docker ps -a
  if [ $? -ne 0 ]; then
